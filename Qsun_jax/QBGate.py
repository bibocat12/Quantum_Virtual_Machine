...
def H(wavefunction, n):
   ...
   # print(f"Amplitudes changing on qubit {n}: {amplitude}, {np.nonzero(amplitude)[0]}")
...
def RX(wavefunction, n, phi=0):
    ...
    for i in np.nonzero(amplitude)[0]:
        if states[i][n] == '0':
            ...
            new_amplitude = new_amplitude.at[i].set(cmath.cos(phi/2)*amplitude[i])
            new_amplitude = new_amplitude.at[i+cut].set(-1j*cmath.sin(phi/2)*amplitude[i])
        else:
            new_amplitude = new_amplitude.at[i].set(cmath.cos(phi/2)*amplitude[i])
            new_amplitude = new_amplitude.at[i-cut].set(-cmath.sin(phi/2)*amplitude[i])
    wavefunction.amplitude = new_amplitude
...
def RZ(wavefunction, n, phi=0):
    ...
    for i in np.nonzero(amplitude)[0]:
        if states[i][n] == '0':
            ...
            new_amplitude = new_amplitude.at[i].set(cmath.exp(-1j*phi/2)*amplitude[i])
        else:
            new_amplitude = new_amplitude.at[i].set(cmath.exp(1j*phi/2)*amplitude[i])
    wavefunction.amplitude = new_amplitude
    ...
    for i in np.nonzero(amplitude)[0]:
        if states[i][control] == '1':
            if states[i][target] == '0':
                # new_amplitude[i+cut] += amplitude[i]
                new_amplitude = new_amplitude.at[i+cut].set(new_amplitude[i+cut] + amplitude[i])
            else:
                new_amplitude = new_amplitude.at[i-cut].set(new_amplitude[i-cut] + amplitude[i])
        else:
            new_amplitude = new_amplitude.at[i].set(amplitude[i])
    wavefunction.amplitude = new_amplitude
